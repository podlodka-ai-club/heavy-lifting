...
class DeliverWorker:
    ...
    def poll_once(self) -> DeliverWorkerReport:
        ...
        try:
            execute_result = self._resolve_delivery_result(task_context=task_context)
            ...
            if delivery is None:
                ...
                comment_body = delivery.comment_body or self._build_comment_body(
                    execute_result=execute_result
                )
                links = self._build_links(execute_result=execute_result, extra_links=delivery.links)
            logger.info(
                "delivery_started",
            ...
            if delivery is None:
                ...
                if delivery.tracker_estimate is not None or delivery.tracker_labels:
            ...
            with session_scope(session_factory=self.session_factory) as session:
                ...
                persisted_task.branch_name = execute_result.branch_name or task_context.branch_name
                persisted_task.pr_url = execute_result.pr_url or task_context.pr_url
