...
class LocalAgentRunner:
    ...
    def run(self, request: AgentRunRequest) -> AgentRunResult:
        ...
        if self._is_triage(request):
            return self._build_triage_result(request=request, logger=logger)
    ...
    def _is_triage(self, request: AgentRunRequest) -> bool:
    ...
    def _build_triage_result(
        self, *, request: AgentRunRequest, logger: Any
    ) -> AgentRunResult:
        usage = self.token_cost_service.with_estimated_cost(self._build_token_usage(request))
        ...
        raw_stdout = self._build_triage_stdout(request.task_context)
        logger.info(
            "agent_run_finished",
            token_usage_entries=1,
            estimated_cost_usd=str(usage.cost_usd),
        ...
        return AgentRunResult(payload=payload, raw_stdout=raw_stdout, raw_stderr="")

    def _build_triage_stdout(self, context: EffectiveTaskContext) -> str:
        tracker = context.tracker_context
        title = (tracker.title if tracker is not None else "") or "Untitled task"
        description = (tracker.description if tracker is not None else "") or ""
        ...
        if description_length >= 1000:
            ...
            markdown_body = (
                ...
                f"{title}\n\n"
                "### Аналитический срез\n"
                "Описание содержит избыточный контекст без чёткой технической ясности.\n\n"
            ...
            markdown_body = (
                ...
                "**Assigned Story Points:** 3\n"
                "**Generated by:** AI Triage Gatekeeper\n\n"
                "### 1. Intent\n"
                f"- {title}\n\n"
                "### 2. Execution Design\n"
                "1. Реализовать требуемые изменения по описанию.\n\n"
            ...
            markdown_body = (
                ...
                f"- {title}\n\n"
                "### 2. Execution Design\n"
                "1. Реализовать требуемое изменение.\n\n"
                "### 3. Architectural Constraints\n"
                "- Без изменения публичного API.\n\n"
                "### 4. Context\n"
